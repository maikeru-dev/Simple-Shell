...
struct Command {
  enum { BUILTIN, EXECUTE, ALIAS_COMMAND } type;
  int length;
  char *fullCommand;
  char **tokens;
  int (*fn)(int, char **, struct Command **, int *, struct Command **, int *,
            struct Command **, int *);
...
_createBuiltInCommand(char *input,
                      int (*fn)(int argc, char **argv, Command **aliases,
                                int *aliasC, Command **history, int *historyC,
                                Command **builtInCommands, int *builtInC));
